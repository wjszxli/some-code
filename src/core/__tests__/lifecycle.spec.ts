...
import { LifeCycleTypes } from "../type";
...
describe("FormLifeCycle", () => {
  test("handler", () => {
    ...
    expect(cb).toBeCalledWith(
      { payload: data, type: LifeCycleTypes.ON_FORM_INIT },
      undefined
    );
    lifeCycle.notify(LifeCycleTypes.ON_FIELD_INITIAL_VALUE_CHANGE, data);
    expect(cb).toBeCalledTimes(2);
    expect(cb).toBeCalledWith(
      { payload: data, type: LifeCycleTypes.ON_FIELD_INITIAL_VALUE_CHANGE },
      undefined
    );
  });
  ...
  test("type/handler", () => {
    ...
    const lifeCycle2 = new FormLifeCycle(
      LifeCycleTypes.ON_FIELD_INITIAL_VALUE_CHANGE,
      cb2
    );
...
describe("FormHeart", () => {
  ...
  test("lifecycles constructor handler", () => {
    const cb = jest.fn();
    const heart = new FormHeart({
      lifecycles: [new FormLifeCycle(cb)],
    });
    const data = { hello: "world" };
